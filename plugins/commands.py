...
from info import CHANNELS, ADMINS, AUTH_CHANNEL, LOG_CHANNEL, PICS, BATCH_FILE_CAPTION, CUSTOM_FILE_CAPTION, PROTECT_CONTENT, CHNL_LNK, GRP_LNK, REQST_CHANNEL, SUPPORT_CHAT_ID, SUPPORT_CHAT, MAX_B_TN, VERIFY, SHORTLINK_API, SHORTLINK_URL, TUTORIAL, IS_TUTORIAL, PREMIUM_USER, MAIN_CHANNEL, LOGIN_CHANNEL, ADMIN
from utils import get_settings, get_size, is_subscribed, save_group_settings, temp, verify_user, check_token, check_verification, get_token, get_shortlink, get_tutorial, mute_login
        ...
        btn = [
            [
                InlineKeyboardButton(
                    "❆ Jᴏɪɴ Oᴜʀ Cʜᴀɴɴᴇʟ ❆", url="https://t.me/+7oxSIxY4X0c2ZGVl"
                )
            ]
        ]
                    ...
                    reply_markup=InlineKeyboardMarkup(
                        [
                         [
                          ...
                          InlineKeyboardButton("Mᴏᴠɪᴇ Rᴇᴏ̨ᴜᴇsᴛ Gʀᴏᴜᴘ", url="https://t.me/NasraniSeries")
                         ]
                        ]
                    )
                )
                ...
                reply_markup=InlineKeyboardMarkup(
                    [
                     [
                      ...
                      InlineKeyboardButton("Mᴏᴠɪᴇ Rᴇᴏ̨ᴜᴇsᴛ Gʀᴏᴜᴘ", url=MAIN_CHANNEL)
                     ]
                    ]
                )
            )
...
@Client.on_message(filters.command('deleteall') & filters.user(ADMINS) & filters.incoming)
...
@Client.on_message(filters.private & filters.text & filters.command('report')) # & filters.reply)
...
#    lg_cd = lgcd[1].lower().replace(" ", "")
    try:   
        if message.from_user.id == ADMIN: 
            await rreplay_media(client, message)
            return
#        await message.reply_text(
